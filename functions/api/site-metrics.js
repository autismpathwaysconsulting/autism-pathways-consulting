...
export async function onRequest({ request, env }) {
  ...
  if (Number(request.headers.get("Content-Length") || 0) > 256) return metricResponse(413);
  ...
  try {
    ...
    while (true) {
      ...
      if (size > 256) { await reader.cancel(); return metricResponse(413); }
  ...
  try {
    const statements = value.page === "quick_check"
      ? [
          env.APC_SITE_METRICS_DB.prepare("INSERT INTO quick_check_counts (day, event, source, medium, campaign, count) VALUES (date('now'), ?, ?, ?, ?, 1) ON CONFLICT(day, event, source, medium, campaign) DO UPDATE SET count = min(count + 1, 1000000)").bind(value.event, value.attribution.source, value.attribution.medium, value.attribution.campaign),
          env.APC_SITE_METRICS_DB.prepare("DELETE FROM quick_check_counts WHERE day < date('now', '-89 days')"),
        ]
      : [
    ...
    await env.APC_SITE_METRICS_DB.batch(statements);
