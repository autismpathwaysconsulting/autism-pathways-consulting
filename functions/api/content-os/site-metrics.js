...
export async function onRequest({ request, env }) {
  ...
  try {
    const { results } = await env.APC_SITE_METRICS_DB.prepare("SELECT page, event, SUM(count) AS count FROM daily_counts WHERE day >= date('now', '-29 days') GROUP BY page, event ORDER BY page, event LIMIT 40").all();
