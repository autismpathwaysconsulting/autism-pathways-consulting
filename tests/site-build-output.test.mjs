...
test("checked-in Pages bundle contains protected operator routes", async () => {
  ...
  const productionInbox = /href="\/content-os\/calm-feedback\/"/;
  assert.match(contentOs, productionInbox);
  assert.match(practice, productionInbox);
  assert.doesNotMatch(contentOs, /dash\.cloudflare\.com\/.*\/studio/);
  assert.doesNotMatch(practice, /dash\.cloudflare\.com\/.*\/studio/);
});
