...
function renderMetrics() {
  ...
  const drafts = state.data.sessions.filter((session) => session.documentStatus === "DRAFT" && (session.parentSummary || session.actionPlan)).length;
  const queued = state.data.exports.filter((item) => item.destination === "GOOGLE_DRIVE" && item.status === "QUEUED").length;
  const needsReview = clients.filter((client) => ["RECORD_REVIEW_REQUIRED", "DOCUMENTATION_DRAFT"].includes(client.stage)).length;
  element("practiceMetrics").innerHTML = [
    [clients.length, "Active client records"], [needsReview, "Need review"], [drafts, "Document drafts"], [queued, "Drive copies to confirm"],
...
function renderClients() {
  ...
  element("clientList").innerHTML = clients.length ? clients.map((client) => `
    <article class="card operator-card${client.caseId === state.activeCaseId ? " selected-card" : ""}">
      ...
      <dl class="operator-facts"><div><dt>Service</dt><dd>${escapeHtml(client.serviceCode)}</dd></div><div><dt>Source</dt><dd>${escapeHtml(client.sourceStatus.replaceAll("_", " "))}</dd></div><div><dt>Revision</dt><dd>${client.revision}</dd></div></dl>
    ...
    </article>`).join("") : `<div class="card empty-state"><h3>No cloud client records yet</h3><p>Add a new client or migrate the two protected local records after the production database is ready.</p></div>`;
}
...
function renderClientWorkspace() {
  ...
  element("addSession").disabled = state.data.writesEnabled !== true;
  const sessions = state.data.sessions.filter((session) => session.caseId === client.caseId).sort((a, b) => a.sessionNumber - b.sessionNumber);
  element("clientWorkspace").innerHTML = clientForm(client) + `<div class="session-strip" aria-label="Sessions">${sessions.length ? sessions.map((session) => `<button type="button" class="button ${session.sessionId === state.activeSessionId ? "" : "secondary"} compact" data-open-session="${session.sessionId}">Session ${session.sessionNumber} · ${escapeHtml(session.documentStatus.replaceAll("_", " "))}</button>`).join("") : "<p>No sessions yet.</p>"}</div>`;
}
...
function sessionForm(session) {
  ...
  const options = (items, selected) => items.map((value) => `<option value="${value}"${value === selected ? " selected" : ""}>${value.replaceAll("_", " ")}</option>`).join("");
  return `<form id="sessionEditForm" class="card operator-form" data-session-id="${session.sessionId}" data-revision="${session.revision}">
    <div class="card-heading"><div><p class="card-label">Session ${session.sessionNumber}</p><h3>Preparation, notes and parent follow-through</h3></div><span class="status-badge">Revision ${session.revision}</span></div>
    <div class="form-grid">
      <label>Session status<select name="status">${options(sessionStatuses, session.status)}</select></label>
      <label>Document status<select name="documentStatus">${options(documentStatuses, session.documentStatus)}</select></label>
      <label>Scheduled date<input name="scheduledAt" type="date" value="${escapeHtml((session.scheduledAt || "").slice(0, 10))}"></label>
      <label>Meeting date<input name="occurredAt" type="date" value="${escapeHtml((session.occurredAt || "").slice(0, 10))}"></label>
      <label class="span-two">Pre-meeting preparation<textarea name="preparation" rows="4" maxlength="10000">${escapeHtml(session.preparation)}</textarea></label>
      <label class="span-two">Private working notes <span>not included in the parent export</span><textarea name="privateNotes" rows="7" maxlength="30000">${escapeHtml(session.privateNotes)}</textarea></label>
...
function renderActivity() {
  ...
  element("activityList").innerHTML = activity.length ? activity.map((item) => `
    <article class="card operator-card">
      <div class="card-heading"><div><p class="card-label">${escapeHtml(item.recordType)} · ${escapeHtml(item.caseId)}</p><h3>${escapeHtml(item.eventType.replaceAll("_", " "))}</h3></div><span class="status-badge">Revision ${item.revision}</span></div>
...
element("addSession").addEventListener("click", async () => {
  ...
  try {
    ...
    render();
    toast("Session workspace created.");
...
document.addEventListener("click", async (event) => {
  ...
  if (target.dataset.openClient) {
    ...
    const sessions = state.data.sessions.filter((session) => session.caseId === state.activeCaseId).sort((a, b) => a.sessionNumber - b.sessionNumber);
    state.activeSessionId = sessions.at(-1)?.sessionId || null;
    render();
  ...
  try {
    if (form.id === "newClientForm") {
      ...
      state.data = await api({ action: "save_session", sessionId: form.dataset.sessionId, expectedRevision: Number(form.dataset.revision), session: {
        status: String(formData.get("status") || ""), scheduledAt: date("scheduledAt"), occurredAt: date("occurredAt"), preparation: String(formData.get("preparation") || ""), privateNotes: String(formData.get("privateNotes") || ""), parentSummary: String(formData.get("parentSummary") || ""), actionPlan: String(formData.get("actionPlan") || ""), documentStatus: String(formData.get("documentStatus") || ""),
      } });
