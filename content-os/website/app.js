...
const events = { page_view: "Page views", booking_click: "Booking clicks", calendar_open: "Calendar opens", booking_submitted: "Submitted bookings (embed)", form_start: "Form starts", form_submit: "Form submissions", download_click: "Download clicks", parent_support_click: "Parent support clicks" };
  ...
  try {
    ...
    for (const [page, label] of Object.entries(pages)) {
      ...
      for (const [event, title] of Object.entries(events)) {
        if (page === "big_reactions" && !["page_view", "form_start", "form_submit", "download_click", "parent_support_click"].includes(event)) continue;
        if (page !== "big_reactions" && ["form_start", "form_submit", "download_click", "parent_support_click"].includes(event)) continue;
