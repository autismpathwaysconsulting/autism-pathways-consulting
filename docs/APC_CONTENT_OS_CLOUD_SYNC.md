...
The immutable episode ID remains the database identity used by artifacts, reviews, publications and analytics. A separate private episode number can be edited for CJ's internal ordering without rekeying that history. Active private numbers must be unique; an archived episode cannot be restored while its number is assigned to another active episode. Episode IDs and private numbers are excluded from generated spoken words, on-screen text, overlays, source pills, public captions, titles, descriptions and viewer-facing filenames.
