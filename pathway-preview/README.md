...
- The configured preview branch must exactly match `APC_PATHWAY_PREVIEW_BRANCH`; other branch deployments fail closed.
...
- Login accepts only same-origin URL-encoded posts, streams a maximum 4096-byte body, and rejects duplicate or unexpected fields. Protected assets are read-only.
- Preview sessions expire after one hour, have an explicit sign-out action, and can all be revoked by incrementing `APC_PATHWAY_PREVIEW_SESSION_VERSION`.
...
The machine-readable [`portal-technical-readiness.json`](docs/portal-technical-readiness.json) separates tested synthetic controls from external decisions. Its 9.5 score applies only to the bounded synthetic preview, not to production readiness. Every production gate in [`portal-release-gates.json`](docs/portal-release-gates.json) remains blocked.
...
Set project secrets `APC_PATHWAY_PREVIEW_INVITE_SHA256` and `APC_PATHWAY_PREVIEW_SESSION_SECRET`, set the non-secret preview branch and session version variables, build with `npm run build:pathway-preview`, then deploy `pathway-preview/dist` only to that branch of the separate Pages project. Increment the session version to revoke all preview sessions. Never attach the production hostname during preview review.
